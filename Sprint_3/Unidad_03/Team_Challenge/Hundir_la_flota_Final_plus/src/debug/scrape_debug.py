...
st.set_page_config(page_title="Scraper EDHTop16 con Log Detallado", layout="wide")
...
st.title("📋 Scraping con Log Detallado de Mazos Moxfield")
...
if st.button("🧪 Iniciar con Log detallado"):
    torneos = obtener_lista_torneos()
    st.info(f"📅 Torneos encontrados: {len(torneos)}")
    ...
    st.success(f"✅ Log generado: {total_guardados} mazos públicos guardados de {total_mazos} detectados.")
    st.download_button("📥 Descargar Log CSV", data=pd.DataFrame(log).to_csv(index=False), file_name="log_scraping_mazos.csv")
